...
#from pyrcpsp.PSPLIBReader import read_precedence_relation, read_resources, split_dictionary, find_resources_prefixes, find_resources_prefixes_paragraph, find_modes_duration, parse_modes_paragraph_header, PSPLibParsingError, read_modes_paragraph, find_modes_demand, split_list_of_strings_by_predicate
...
class TestRead_precedence_relation(TestCase):
    def test_read_precedence_relation(self):
        ...
        expected_dict, expected_dummy_start, expected_dummy_end = {'1':['2','3','4'],'2':['5','10'], '10': ['11'],
                                                                   '5': ['11'], '3': ['11'], '4': ['11']}, '1', '11'
        result_dict, result_dummy_start, result_dummy_end = PSPLIBReader.read_precedence_relation(l1)
    ...
    def test_read_resources(self):
        ...
        self.assertEqual(PSPLIBReader.read_resources(label_line, capacity_line), {'R1': 20, 'R2': 26, 'N1': 23, 'N2': 36})

    def test_split_dictionary(self):
        input_dict = {"some1": 4, "some5": 2, "other5": 9}
        list_of_prefixes= ["some","other"]
        self.assertEqual(PSPLIBReader.split_dictionary(input_dict, list_of_prefixes), [{"some1": 4, "some5": 2}, {"other5": 9}])
    ...
    def test_find_resources_prefixes(self):
        ...
        self.assertEqual(PSPLIBReader.find_resources_prefixes(lines), ['R', 'N', 'D'])
    ...
    def test_find_resources_prefixes_paragraph(self):
        lines = ["projects                      :  1",
                 ...
                 "  - nonrenewable              :  2   N",
                 "  - doubly constrained        :  0   D"]
        self.assertEqual(PSPLIBReader.find_resources_prefixes_paragraph(lines), ['R', 'N', 'D'])
    ...
    def test_find_modes_duration(self):
        ...
        self.assertEqual(PSPLIBReader.find_modes_duration(lines), expected_result)
    ...
    def test_find_modes_duration_lacking_mode_causes_exception(self):
        ...
        with self.assertRaises(PSPLIBReader.PSPLibParsingError):
            PSPLIBReader.find_modes_duration(lines)

    def test_parse_modes_paragraph_header(self):
        input_line = "jobnr. mode duration  R 1  R 2  N 1  N 2"
        resource_sequence = PSPLIBReader.parse_modes_paragraph_header(input_line)
    ...
    def test_parse_modes_paragraph_header_raise_when_not_known_header(self):
        ...
        with self.assertRaises(PSPLIBReader.PSPLibParsingError):
            PSPLIBReader.parse_modes_paragraph_header(input_line)
    ...
    def test_read_modes_paragraph(self):
        ...
        result = PSPLIBReader.read_modes_paragraph(list_of_paragraph_lines)
    ...
    def test_find_modes_demand(self):
        ...
        self.assertEqual(PSPLIBReader.find_modes_demand(lines), expected_result)

    def test_split_list_of_strings_by_predicate(self):
        lines = ['cfg', '***','ab','cd','***', 'ij', 'kl', '***', 'cfg']
        iters = PSPLIBReader.split_list_of_strings_by_predicate(lines, lambda x : '***' in x)
