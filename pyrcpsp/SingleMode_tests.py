...
#from pyrcpsp.SingleModeClasses import Problem, Solution, Activity

#from pyrcpsp.GeneticAlgorithmSolver import crossover_sgs_nonrandom, SerialScheduleGenerationSchemeGenerator
...
class Test(unittest.TestCase):
    ...
    def setUp(self): #funkcja ktora framework testowy bedzie wykonywala przed kazda funkcja testowa
        self.activity1 = SingleModeClasses.Activity("a1",3,{1:2})
        self.activity2 = SingleModeClasses.Activity("a2",4,{1:3})
        self.activity3= SingleModeClasses.Activity("a3",2,{1:4})
        self.activity4= SingleModeClasses.Activity("a4",2,{1:4})
        self.activity5= SingleModeClasses.Activity("a5",1,{1:3})
        self.activity6= SingleModeClasses.Activity("a6",4,{1:2})
        
        activity_graph = {SingleModeClasses.Activity.DUMMY_START:[self.activity1,self.activity2],
                          self.activity1:[self.activity3], 
                          self.activity3:[self.activity5], 
                          self.activity2:[self.activity4],
                          self.activity4:[self.activity6],
                          self.activity5:[SingleModeClasses.Activity.DUMMY_END],
                          self.activity6:[SingleModeClasses.Activity.DUMMY_END]}
        
        resources = {1:4}
        
        self.problem = SingleModeClasses.Problem(activity_graph, resources)
        
        self.start_times = SingleModeClasses.Solution()
    ...
    def test_solve(self):
        solver = GeneticAlgorithmSolver.GeneticAlgorithmSolver(self.problem)
    ...
    def test_sgs_2_dict(self):
        solution = SingleModeClasses.Solution.generate_solution_from_serial_schedule_generation_scheme(self.sgs, self.problem)
    ...
    def test_sgs_2_dict_2(self):
        ...
        activity_graph = {SingleModeClasses.Activity.DUMMY_START:[activity1],
                          activity1:[activity2], 
                          activity2:[SingleModeClasses.Activity.DUMMY_END]}
        resources = {1:7}
        problem = SingleModeClasses.Problem(activity_graph, resources)
        solution_sgs = SingleModeClasses.Solution.generate_solution_from_serial_schedule_generation_scheme([activity1, activity2], problem)
        self.assertEqual(problem.compute_makespan(solution_sgs), 7, "Makespan is not egual 7")
    
    def test_sgs_2_dict_3(self):             
        solution = SingleModeClasses.Solution.generate_solution_from_serial_schedule_generation_scheme(self.sgs2, self.problem)
        #self.assertEqual(solution, self.start_times, "Expected %s, got %s" % (self.start_times, solution) )  
    ...
    def test_solution_equality(self):
        s = SingleModeClasses.Solution()
        o = SingleModeClasses.Solution()
    ...
    def test_push_ready_activities_to_ready_to_schedule(self):
        activity_graph = {SingleModeClasses.Activity.DUMMY_START: [self.activity1],
                          self.activity1: [self.activity2, self.activity3],
                          self.activity2: [self.activity4],
                          self.activity3: [self.activity4],
                          self.activity4: [SingleModeClasses.Activity.DUMMY_END]}
        problem = SingleModeClasses.Problem(activity_graph, {})
        ...
        generator = GeneticAlgorithmSolver.SerialScheduleGenerationSchemeGenerator(problem)
    ...
    def test_generate_random_sgs_from_problem(self):
        generator = GeneticAlgorithmSolver.SerialScheduleGenerationSchemeGenerator(self.problem)
    ...
    def test_crossover_sgs_nonrandom(self):
        ...
        sgs_dad = [2,4,6,1,3,5]
        q = 3
        sgs_daughter, sgs_son = GeneticAlgorithmSolver.crossover_sgs_nonrandom(sgs_mum, sgs_dad, q)
